...
pub trait VerifiedAdd: SSAMod {
	fn verified_add_op(&mut self, block: Self::ActionRef, opc: MOpcode, vt:
					   ValueType, args: &[Self::ValueRef],
					   addr: Option<u64>) -> Self::ValueRef;
}
...
impl Verify for SSAStorage {
	fn verify_block(&self, block: &NodeIndex) -> VResult<Self> {
		...
		for edge in edges.iter() {
			match self.g[*edge] {
				EdgeData::Control(i) if i < 2 => {
					...
					check!(edges.len() == 2, SSAErr::WrongNumEdges(*block, 2,
																   edges.len()));
		...
		if edges.len() == 2 {
			...
			check!(selector.is_none(), SSAErr::UnexpectedSelector(*block,
																  selector.unwrap()));
		}
	...
	fn verify_expr(&self, exi: &NodeIndex) -> VResult<Self> {
		...
		match *node_data {
			NodeData::Op(opcode, ValueType::Integer { width: w }) => {
				...
				{
					check!(op_len == n, SSAErr::WrongNumOperands(*i, n,
																 op_len));
				}
					...
					_ => {
						...
						for op in operands.iter() {
							let w1 = self.get_node_data(op)
							             .map(&extract)
						                 .unwrap();
							check!(w == w1, SSAErr::IncompatibleWidth(*i, w,
																	  w1));
						}
					},
				}
